/**
 * Seed script: populate CreativeBrain with 100+ donation fundraiser angles
 * Run: npx ts-node scripts/seed-angles.ts
 * Or add to package.json: "seed:angles": "ts-node scripts/seed-angles.ts"
 */
...
async function main() {
    console.log("Seeding Creative Brain angles...");

    const existing = await (prisma as any).creativeBrain.findUnique({
        where: { scope: "fundraiser" },
    });

    if (existing) {
        // Merge new angles with existing ones (deduplicate)
        const existingAngles = (existing.anglesList || "").split("\n").map((s: string) => s.trim()).filter(Boolean);
        const newAngles = ANGLES.split("\n").map((s) => s.trim()).filter(Boolean);
        const allAngles = [...existingAngles];

        let added = 0;
        for (const angle of newAngles) {
            const normalized = angle.toLowerCase().replace(/\s+/g, " ");
            const exists = allAngles.some((a) => a.toLowerCase().replace(/\s+/g, " ") === normalized);
            if (!exists) {
        ...
        await (prisma as any).creativeBrain.update({
            where: { scope: "fundraiser" },
            data: { anglesList: allAngles.join("\n") },
        });

        console.log(`✅ Added ${added} new angles. Total: ${allAngles.filter(a => !a.startsWith("#")).length} angles`);
    } else {
        await (prisma as any).creativeBrain.create({
            data: {
                scope: "fundraiser",
                anglesList: ANGLES,
        ...
        console.log(`✅ Created Creative Brain with ${ANGLES.split("\n").filter(l => l && !l.startsWith("#")).length} angles`);
    }
